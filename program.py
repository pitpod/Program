...
class Application(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.resize(desktop_x - 80, desktop_y - 400)
        wsize = self.size()
        ...
        for index, row in self.riding_name.iterrows():
            ...
            for colindex, colvalue in row.items():
                if colindex != "name":
                    ...
                    if day.weekday() == 6:
                        if hr_times >= weekly:
                            self.hr_pd.at[index,colindex] = self.sd.excel_day(colindex) + self.week_num_fd
                            self.hr_pd.at[index,colindex - 1] = self.sd.excel_day(colindex) + self.week_num_fd - 1
                            self.hr_pd.at[index,colindex - 2] = self.sd.excel_day(colindex) + self.week_num_fd - 2
                            self.hr_pd.at[index,colindex - 3] = self.sd.excel_day(colindex) + self.week_num_fd - 3
                            self.hr_pd.at[index,colindex - 4] = self.sd.excel_day(colindex) + self.week_num_fd - 4
                            self.hr_pd.at[index,colindex - 5] = self.sd.excel_day(colindex) + self.week_num_fd - 5
                            self.hr_pd.at[index,colindex - 6] = self.sd.excel_day(colindex) + self.week_num_fd - 6
                        else:
                            self.hr_pd.at[index,colindex] = 0
                        hr_times = 0
                    else:
                        self.hr_pd.at[index,colindex] = 0
            tp = row.iat[0].replace('(',f'({hr_times_month}/')
            self.riding_name.replace(row.iat[0],tp, inplace=True)
            # nametimes = column.replace('(',f'({hr_times_month}/')
            if hr_times_month > monthly and monthly != 0:
                for h_day in header_serial:
                    self.hr_pd.at[index,h_day] = -1.0
            elif hr_times_month == monthly and monthly != 0:
                for h_day in header_serial:
